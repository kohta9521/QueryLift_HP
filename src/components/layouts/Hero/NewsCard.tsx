import React from "react";

// next
import Link from "next/link";

// icon

// props
export type NewsCardProps = {
  id: string;
  link: string;
  tag: string;
  date: string;
  text: string;
};

const NewsCard = ({ id, link, tag, date, text }: NewsCardProps) => {
  return (
    <div className="absolute bottom-4 left-0 right-0 flex justify-center items-center w-full z-10 px-4">
      <Link
        key={id}
        href={link}
        className="group w-full max-w-md h-auto min-h-[50px] sm:min-h-[60px] bg-white/40 backdrop-blur-md rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between p-2 sm:p-3 sm:px-4 sm:py-3 gap-1 sm:gap-0"
      >
        <div className="flex items-center w-full sm:w-auto">
          <p className="block px-1.5 py-0.5 sm:px-2 sm:py-1 bg-blue-600 text-white text-xs rounded-full whitespace-nowrap">
            {tag}
          </p>
          <p className="text-xs sm:text-sm ml-1.5 sm:ml-2 whitespace-nowrap">
            {date}
          </p>
        </div>
        <div className="flex items-center justify-between w-full sm:w-auto">
          <p className="text-xs sm:text-sm truncate max-w-[calc(100%-20px)] sm:max-w-[60%]">
            {text}
          </p>
          <svg
            width="12"
            height="12"
            viewBox="0 0 15 15"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="group-hover:translate-x-1 transition-transform duration-300 ml-1 flex-shrink-0"
          >
            <path
              d="M6.1584 3.13508C6.35985 2.94621 6.67627 2.95642 6.86514 3.15788L10.6151 7.15788C10.7954 7.3502 10.7954 7.64949 10.6151 7.84182L6.86514 11.8418C6.67627 12.0433 6.35985 12.0535 6.1584 11.8646C5.95694 11.6757 5.94673 11.3593 6.1356 11.1579L9.565 7.49985L6.1356 3.84182C5.94673 3.64036 5.95694 3.32394 6.1584 3.13508Z"
              fill="currentColor"
              fill-rule="evenodd"
              clip-rule="evenodd"
            ></path>
          </svg>
        </div>
      </Link>
    </div>
  );
};

export default NewsCard;
